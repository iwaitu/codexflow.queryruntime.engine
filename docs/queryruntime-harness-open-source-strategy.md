...
| CLI streaming contract | Contract defined; implementation remains planned. Normal `qre run` prints final assistant text only after completion, and `--json` prints one final result JSON object. | `QreCliSmokeTests.Run_JsonOutputIsSingleFinalResultObject`; `QreCliSmokeTests.Run_StreamingAndUnknownOptionsDoNotBecomePromptText`. | `--stream` and `--jsonl-stream` are reserved and fail explicitly until the streaming implementation ships. |
