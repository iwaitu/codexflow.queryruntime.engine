...
namespace CodexFlow.Core.Skills.Node;

public class NodeScaffolderSkill : ISkill
{
    public string Name => "node-scaffolder";
    public string Description => "Initialize a new Node.js (TypeScript) project with Express, Prisma, and Zod.";
    ...
    public async Task<SkillResult> ExecuteAsync(SkillContext context)
    {
        ...
        var packageManager = context.Parameters.GetValueOrDefault("package_manager", "npm")?.ToString() ?? "npm";
        ...
        try
        {
            ...
            var args = $"\"{scriptPath}\" \"{projectName}\" --output \"{workspace}\" --package-manager {packageManager}";
    ...
    private static string ResolveScriptPath()
    {
        ...
        var candidate = Path.Combine(baseDir, "skills", "node-scaffolder", "scripts", "scaffold.py");
        ...
        while (current != null)
        {
            ...
            {
                var slnCandidate = Path.Combine(current.FullName, "CodexFlow", "skills", "node-scaffolder", "scripts", "scaffold.py");
